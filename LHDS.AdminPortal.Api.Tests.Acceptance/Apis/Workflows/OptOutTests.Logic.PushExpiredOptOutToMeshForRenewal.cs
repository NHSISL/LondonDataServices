...
namespace LHDS.AdminPortal.Api.Tests.Acceptance.Apis.OptOuts
{
    public partial class OptOutApiTests
    {
        ...
        public async Task ShouldPushExpiredOptOutToMeshForRenewalAsync()
        {
            ...
            string mexWorkflowId = this.apiBroker.optOutConfiguration.WorkflowId;
            string batchReference = this.dateTimeBroker.GetCurrentDateTimeOffset().ToString("yyyyMMddHHmmss");
            string mexTo = this.apiBroker.optOutConfiguration.To;
            ...
            {
                await this.apiBroker.PostOptOutAsync(optOut);
            }
            ...
            //When
            var actualMessage = await this.apiBroker.PushExpiredOptOutsToMeshForRenewalAsync();
            ...
            foreach (OptOut optOut in outputOptOuts)
            {
                await this.apiBroker.DeleteOptOutByIdAsync(optOut.Id);
            }
