                ...
                if (response.data) {
                    const suppliers = response.data.map((supplier: Supplier) =>
                        new SupplierView(
                            ...
                            supplier.decryptionManualTriggerUrl,
                            supplier.canDecryptIngestionTracking,
                            supplier.canDownloadIngestionTracking,
                            supplier.canRelandIngestionTracking,
                            supplier.createdBy,
            ...
            useEffect(() => {
                if (response.data && response.data[0]) {
                    const supplier = new SupplierView(
                        ...
                        response.data[0].decryptionManualTriggerUrl,
                        response.data[0].canDecryptIngestionTracking,
                        response.data[0].canDownloadIngestionTracking,
                        response.data[0].canRelandIngestionTracking,
