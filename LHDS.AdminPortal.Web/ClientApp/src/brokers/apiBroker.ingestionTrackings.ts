...
class IngestionTrackingBroker {
    ...
    private processOdataResult = (result: AxiosResponse) => {
        const data = result.data.value.map((ingestionTracking: any) => new IngestionTracking(ingestionTracking));
    ...
    async GetAllIngestionTrackingsAsync(queryString: string) {
        var url = this.relativeIngestionTrackingUrl + queryString;

        return await this.apiBroker.GetAsync(url)
            .then(result => result.data.map((ingestionTracking: any) => new IngestionTracking(ingestionTracking)));
    }

    async GetIngestionTrackingFirstPagesAsync(query: string) {
        var url = this.relativeIngestionTrackingOdataUrl + query;
