...
namespace LHDS.Core.Tests.Unit.Services.Coordinations.HealthChecks.TerminologyPolls.FailedToProcessHealthCheck
{
    public partial class TerminologyPollsFailedToProcessHealthCheckServiceTests
    {
        ...
        private readonly TerminologyPollsFailedToProcessHealthCheckService
            terminologyPollsFailedToProcessHealthCheckService;
        ...
        public TerminologyPollsFailedToProcessHealthCheckServiceTests()
        {
            ...
            this.terminologyPollsFailedToProcessHealthCheckService =
        ...
        private static IQueryable<TerminologyPoll> CreateRandomUnhealthyTerminologyPolls(string resourceType)
        {
            DateTimeOffset dateTimeOffset = DateTimeOffset.UtcNow;
            DateTimeOffset lastExpectedPollTime = dateTimeOffset.Date.AddDays(-1).AddHours(23);
            DateTimeOffset unhealthyDateTime = lastExpectedPollTime.AddMinutes((UnHealthyThresholdMinutes + 1) * -1);

            return CreateTerminologyPollFiller(unhealthyDateTime, resourceType)
                .Create(count: GetRandomNumber()).AsQueryable();
        }

        private static IQueryable<TerminologyPoll> CreateRandomDegradedTerminologyPolls(string resourceType)
        {
            DateTimeOffset dateTimeOffset = DateTimeOffset.UtcNow;
            DateTimeOffset lastExpectedPollTime = dateTimeOffset.Date.AddDays(-1).AddHours(23);
            DateTimeOffset degradedDateTime = lastExpectedPollTime.AddMinutes((DegradedThresholdMinutes + 1) * -1);

            return CreateTerminologyPollFiller(degradedDateTime, resourceType)
                .Create(count: GetRandomNumber()).AsQueryable();
        }

        private static IQueryable<TerminologyPoll> CreateRandomHealthyTerminologyPolls(string resourceType)
        {
            DateTimeOffset dateTimeOffset = DateTimeOffset.UtcNow;
            DateTimeOffset healthyDateTime = dateTimeOffset;

            return CreateTerminologyPollFiller(healthyDateTime, resourceType)
                .Create(count: GetRandomNumber()).AsQueryable();
        }
