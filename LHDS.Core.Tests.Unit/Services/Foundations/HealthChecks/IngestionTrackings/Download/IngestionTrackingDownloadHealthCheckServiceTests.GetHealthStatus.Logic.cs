...
namespace LHDS.Core.Tests.Unit.Services.Foundations.HealthChecks.IngestionTrackings.Download
{
    public partial class IngestionTrackingDownloadHealthCheckServiceTests
    {
        ...
        {
            // given
            string checkName = "downloadQueue";
            DateTimeOffset randomDateTimeOffset = DateTimeOffset.UtcNow;
            int randomNumber = GetRandomNumber();

            int degradedThresholdMinutes = this.inMemoryConfiguration
                .GetValue("HealthChecks:IngestionTracking:Download:DegradedThreshold", 1440);

            int unHealthyThresholdMinutes = this.inMemoryConfiguration
                .GetValue("HealthChecks:IngestionTracking:Download:UnHealthyThreshold", 2880);
        ...
        public async Task ShouldGetHealthStatusAsDegradedAsync()
        {
            ...
            string message =
                $"{randomNumber} files have not been downloaded. Please check logs and function status.";

            var values = new Dictionary<string, object>
            {
                { "description", "Download Queue" },
                { "notDownloadedItems", randomNumber },
        ...
        [Fact]
        public async Task ShouldExcludeDeletedFilesFromDownloadQueueAsync()
        {
            // given
            string checkName = "downloadQueue";
            DateTimeOffset randomDateTimeOffset = DateTimeOffset.UtcNow;
