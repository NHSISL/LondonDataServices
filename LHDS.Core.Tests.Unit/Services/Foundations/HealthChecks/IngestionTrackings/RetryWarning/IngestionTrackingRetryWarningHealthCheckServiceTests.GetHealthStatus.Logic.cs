...
namespace LHDS.Core.Tests.Unit.Services.Foundations.HealthChecks.IngestionTrackings.RetryWarning
{
    public partial class IngestionTrackingRetryWarningHealthCheckServiceTests
    {
        ...
        public async Task ShouldGetHealthStatusAsHealthyAsync()
        {
            ...
            int maxRetryCount = this.inMemoryConfiguration
            ...
            string message = "Nothing to warn about. All up to date.";
        ...
        public async Task ShouldGetHealthStatusAsDegradedAsync()
        {
            ...
            string message =
                $"{randomNumber} files have had retries but have not yet failed. " +
                $"Please check logs and function status.";

            var values = new Dictionary<string, object>
            {
                { "description", CheckDescriptionName },
                { "retryWarningItems", randomNumber },
        ...
        [Fact]
        public async Task ShouldExcludeZeroRetryRecordsFromWarningCountAsync()
        {
            // given
            DateTimeOffset randomDateTimeOffset = DateTimeOffset.UtcNow;
            int randomNumber = GetRandomNumber();
            ...
            List<IngestionTracking> noRetryRecords = CreateRandomIngestionTrackings(
                dateTimeOffset: randomDateTimeOffset.AddMinutes(-unHealthyThresholdMinutes).AddSeconds(-1),
                retryCount: 0,
                count: randomNumber);
            ...
            this.storageBrokerMock.Setup(broker =>
                broker.SelectAllIngestionTrackingsAsync())
                    .ReturnsAsync(noRetryRecords.AsQueryable());
        ...
        [Fact]
        public async Task ShouldExcludeExhaustedRetryRecordsFromWarningCountAsync()
        {
            // given
            DateTimeOffset randomDateTimeOffset = DateTimeOffset.UtcNow;
            int randomNumber = GetRandomNumber();
            ...
            List<IngestionTracking> exhaustedRetryRecords = CreateRandomIngestionTrackings(
                dateTimeOffset: randomDateTimeOffset.AddMinutes(-unHealthyThresholdMinutes).AddSeconds(-1),
            ...
            this.storageBrokerMock.Setup(broker =>
                broker.SelectAllIngestionTrackingsAsync())
                    .ReturnsAsync(exhaustedRetryRecords.AsQueryable());
