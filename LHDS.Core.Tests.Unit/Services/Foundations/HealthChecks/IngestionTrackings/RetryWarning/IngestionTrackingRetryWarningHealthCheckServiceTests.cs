...
namespace LHDS.Core.Tests.Unit.Services.Foundations.HealthChecks.IngestionTrackings.RetryWarning
{
    public partial class IngestionTrackingRetryWarningHealthCheckServiceTests
    {
        ...
        private const string ConfigSectionName = "HealthChecks:IngestionTracking:RetryWarning";

        public IngestionTrackingRetryWarningHealthCheckServiceTests()
        {
            ...
            {
                { $"{ConfigSectionName}:MaxRetryCount", "3" },
            ...
            this.ingestionTrackingHealthItemService = new IngestionTrackingRetryWarningHealthCheckService(
        ...
        private static List<IngestionTracking> CreateRandomIngestionTrackings(
            ...
            int count)
        {
            return CreateIngestionTrackingFiller(dateTimeOffset, retryCount)
                .Create(count)
                    .ToList();
        }
