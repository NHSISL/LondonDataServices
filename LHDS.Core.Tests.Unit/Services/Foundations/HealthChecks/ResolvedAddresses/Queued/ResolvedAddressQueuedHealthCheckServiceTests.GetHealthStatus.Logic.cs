...
namespace LHDS.Core.Tests.Unit.Services.Foundations.HealthChecks.ResolvedAddresses.Queued
{
    public partial class ResolvedAddressQueuedHealthCheckServiceTests
    {
        ...
        public async Task ShouldGetHealthStatusAsHealthyAsync()
        {
            ...
            string message = "Nothing queued. All up to date.";
        ...
        public async Task ShouldGetHealthStatusAsDegradedAsync()
        {
            ...
            string message =
                $"{randomNumber} records are queued but have not been processed. " +
        ...
        [Fact]
        public async Task ShouldExcludeProcessedRecordsFromQueuedCountAsync()
        {
            // given
            DateTimeOffset randomDateTimeOffset = DateTimeOffset.UtcNow;
            int randomNumber = GetRandomNumber();
            ...
            List<ResolvedAddress> processedRecords = CreateRandomResolvedAddresses(
                dateTimeOffset: randomDateTimeOffset.AddMinutes(-unHealthyThresholdMinutes).AddSeconds(-1),
                isProcessing: false,
                isProcessed: true,
                count: randomNumber);
            ...
            this.storageBrokerMock.Setup(broker =>
                broker.SelectAllResolvedAddressesAsync())
                    .ReturnsAsync(processedRecords.AsQueryable());
