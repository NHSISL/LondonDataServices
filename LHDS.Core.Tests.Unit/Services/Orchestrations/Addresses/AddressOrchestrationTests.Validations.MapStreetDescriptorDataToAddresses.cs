...
namespace LHDS.Core.Tests.Unit.Services.Orchestrations.Addresses
{
    ...
    {
        [Fact]
        public async Task ShouldThrowValidationExceptionOnMapStreetDescriptorDataIfFileDoesNotExistAsync()
        {
            // Given
            ...
            // When
            ValueTask<List<Address>> mapStreetDescriptorDataTask =
                service.MapStreetDescriptorDataToAddressesAsync(invalidFileName);
            ...
            InvalidFileAddressOrchestrationException actualFileAddressOrchestrationException =
                await Assert.ThrowsAsync<InvalidFileAddressOrchestrationException>(
                    mapStreetDescriptorDataTask.AsTask);
