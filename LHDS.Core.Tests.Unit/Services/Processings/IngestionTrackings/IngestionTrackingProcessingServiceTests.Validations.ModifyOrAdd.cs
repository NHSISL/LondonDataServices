...
namespace LHDS.Core.Tests.Unit.Services.Processings.IngestionTrackings
{
    ...
    {
        [Fact]
        public async Task ShouldThrowValidationExceptionsOnModifyOrAddIfIngestionTrackingProcessingIsNullAndLogItAsync()
        {
            // given
            ...
            ValueTask<IngestionTracking> AddIngestionTrackingTask =
                this.ingestionTrackingProcessingService.ModifyOrAddIngestionTrackingAsync(nullIngestionTracking);
        ...
        public async Task ShouldThrowValidationExceptionsOnModifyOrAddIfIdIsInvalidAndLogItAsync()
        {
            ...
            ValueTask<IngestionTracking> RetrieveIngestionTrackingTask =
                this.ingestionTrackingProcessingService.ModifyOrAddIngestionTrackingAsync(invalidIngestionTracking);

            IngestionTrackingProcessingValidationException actualIngestionTrackingProcessingValidationException =
                await Assert.ThrowsAsync<IngestionTrackingProcessingValidationException>(RetrieveIngestionTrackingTask.AsTask);
