...
namespace LHDS.Core.Tests.Unit.Services.Processings.TerminologyArtifacts
{
    ...
    {
        [Fact]
        public async Task ShouldGetNonDownloadedUserTerminologyArtifact()
        {
            // given
            ...
            artifactsList.Last().IsForUser = true;
            artifactsList.Last().IsDownloadedForUser = false;
            ...
            TerminologyArtifact? actualTerminologyArtifact =
                await this.terminologyArtifactProcessingService.GetNonDownloadedUserArtifactAsync();
