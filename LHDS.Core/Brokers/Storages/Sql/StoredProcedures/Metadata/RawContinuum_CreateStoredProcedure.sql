CREATE OR ALTER PROCEDURE [Metadata].[RawContinuum_CreateStoredProcedure] @SpecificationObjectId [uniqueidentifier] AS

BEGIN

SET NOCOUNT ON;

--DECLARE @SpecificationObjectId UNIQUEIDENTIFIER = '28A1F2DF-0DEF-46DA-B319-A08D29B9C7B6';
...
FROM [Configuration].[Objects_GetBaseMetadata](@SpecificationObjectId)
...
--IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(@RawContinuumProcedureName) AND type in (N'P'))
	...
	--EXEC [Configuration].[ExecSql] @SpecificationObjectId, 'RawContinuum_CreateStoredProcedure', @SQL_RawContinuum_Proc_Template;
