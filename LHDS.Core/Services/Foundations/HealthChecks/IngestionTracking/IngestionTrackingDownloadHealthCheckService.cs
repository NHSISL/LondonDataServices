...
namespace LHDS.Core.Services.Foundations.HealthChecks.IngestionTracking
{
    public partial class IngestionTrackingDownloadHealthCheckService : IIngestionTrackingHealthItemService
    {
        ...
        private const string CheckName = "downloadQueue";
        private const string CheckDescriptionName = "Download Queue";
        private const string ConfigSectionName = "HealthChecks:IngestionTracking:Download";

        public IngestionTrackingDownloadHealthCheckService(
        ...
        public ValueTask<HealthCheckResult> GetHealthStatusAsync() =>
            TryCatch(async () =>
            {
                ...
                string message = totalCount == 0
                    ...
                    : $"{totalCount} files have not been downloaded. Please check logs and function status.";

                var values = new Dictionary<string, object>
                {
                    { "description", CheckDescriptionName },
                    { "notDownloadedItems", totalCount },
