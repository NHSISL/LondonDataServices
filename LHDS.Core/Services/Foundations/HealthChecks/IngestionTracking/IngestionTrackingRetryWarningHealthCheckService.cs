...
namespace LHDS.Core.Services.Foundations.HealthChecks.IngestionTracking
{
    public partial class IngestionTrackingRetryWarningHealthCheckService : IIngestionTrackingHealthItemService
    {
        ...
        public IngestionTrackingRetryWarningHealthCheckService(
        ...
        public ValueTask<HealthCheckResult> GetHealthStatusAsync() =>
            TryCatch(async () =>
            {
                ...
                var filteredQuery = ingestionTrackingQuery.Where(ingestionTracking =>
                    ingestionTracking.RetryCount > 0 && ingestionTracking.RetryCount < maxRetryCount);
                ...
                string message = totalCount == 0
                    ...
                    : $"{totalCount} files have had retries but have not yet failed. " +
