...
namespace LHDS.Core.Services.Foundations.HealthChecks.ResolvedAddress
{
    public partial class ResolvedAddressExportHealthCheckService : IResolvedAddressHealthItemService
    {
        ...
        private const string CheckName = "queuedForExport";
        private const string CheckDescriptionName = "Queued For Export";
        private const string ConfigSectionName = "HealthChecks:ResolvedAddress:Export";

        public ResolvedAddressExportHealthCheckService(
        ...
        public ValueTask<HealthCheckResult> GetHealthStatusAsync() =>
            TryCatch(async () =>
            {
                ...
                var filteredQuery = resolvedAddressQuery.Where(resolvedAddress =>
                    resolvedAddress.IsProcessed && !resolvedAddress.IsExported);
                ...
                string message = totalCount == 0
                    ? $"Nothing queued for export. All up to date."
                    : $"{totalCount} records are processed but have not been exported. " +
                        $"Please check logs and function status.";

                var vals = new Dictionary<string, object>
                {
                    { "description", CheckDescriptionName },
                    { "queuedItems", totalCount },
