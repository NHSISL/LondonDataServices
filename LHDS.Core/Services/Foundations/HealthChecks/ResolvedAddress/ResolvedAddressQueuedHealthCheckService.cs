...
namespace LHDS.Core.Services.Foundations.HealthChecks.ResolvedAddress
{
    public partial class ResolvedAddressQueuedHealthCheckService : IResolvedAddressHealthItemService
    {
        ...
        private const string CheckName = "queuedForProcessing";
        private const string CheckDescriptionName = "Queued For Processing";
        private const string ConfigSectionName = "HealthChecks:ResolvedAddress:Queued";

        public ResolvedAddressQueuedHealthCheckService(
        ...
        public ValueTask<HealthCheckResult> GetHealthStatusAsync() =>
            TryCatch(async () =>
            {
                ...
                var filteredQuery = resolvedAddressQuery.Where(resolvedAddress =>
                    !resolvedAddress.IsProcessing && !resolvedAddress.IsProcessed);
                ...
                string message = totalCount == 0
                    ? $"Nothing queued. All up to date."
                    : $"{totalCount} records are queued but have not been processed. " +
