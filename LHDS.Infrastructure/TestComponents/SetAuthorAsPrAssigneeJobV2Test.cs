...
// ---------------------------------------------------------------------------
//
// TEST COMPONENT ONLY — local copy of the proposed ADotNet SetAuthorAsPrAssigneeJobV2
// with a try/catch around addAssignees (logs and moves on instead of failing when the
// assignee is already taken). Used to test the behaviour on a branch before an ADotNet
// release. Remove once ADotNet ships the updated component.
...
namespace LHDS.Infrastructure.TestComponents
{
    public sealed class SetAuthorAsPrAssigneeJobV2Test : Job
    {
        public SetAuthorAsPrAssigneeJobV2Test(string runsOn)
        {
            RunsOn = runsOn;
